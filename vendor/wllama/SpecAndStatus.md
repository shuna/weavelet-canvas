...
| `single-thread-webgpu-asyncify-compat.wasm` | WebGPU | wasm32 compat | あり | 不要 (Asyncify) | 2 GB | JSPI なし + WebGPU 環境 (Firefox, Android Chrome 等) | **experimental, disabled** |
| `multi-thread-webgpu-asyncify-compat.wasm` | WebGPU | wasm32 compat | あり | 不要 (Asyncify) | 2 GB | COOP/COEP + JSPI なし + WebGPU MT | **experimental, disabled** |
...
- **Asyncify WebGPU バリアント** (`*-webgpu-asyncify-compat.wasm`) は `-sASYNCIFY=1` で JSPI なしに WebGPU を使う。ビルドリスクあり（`-fwasm-exceptions` との相性未確認）のため **初期状態では disabled**。E2E 検証後に active 化を判断する。
...
| active 化判断 | Chromium ✓。Firefox Desktop での検証が残り（JSPI なし + WebGPU ありの典型環境）。**Firefox で確認後に `disabled: true` を除去して active 化する** |
...
**Firefox での active 化手順**:
1. Firefox で `tests/webgpu-asyncify-verify.spec.ts` を実行し ST/MT とも PASS を確認
2. `src/vendor/wllama/variant-table.ts` の両 Asyncify エントリから `disabled: true` を除去
3. `SpecAndStatus.md` に Firefox 結果を追記してこの行を削除
