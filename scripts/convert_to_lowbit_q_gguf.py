#!/usr/bin/env python3
"""
Reference converter: standard GGUF → lowbit-Q GGUF.

Reads a standard GGUF file (F32, F16, Q8_0), decomposes weight tensors
into the OneBit (arXiv:2402.11295) SVID representation (a, b, sign),
...
def convert(input_path: str, output_path: str, verify: bool = False):
    ...
    for tensor in header["tensors"]:
        ...
        if is_weight_tensor(name):
            ...
            a, b, sign_packed = decompose_lowbit_q(fp32, out_features, in_features)

            if verify:
                W_recon = reconstruct_lowbit_q(a, b, sign_packed, out_features, in_features)
            ...
            if m:
                lowbit_q_layer_indices.add(int(m.group(1)))
    ...
    metadata["lowbit-q.version"] = (GGUF_TYPE_UINT32, 1)
    metadata["lowbit-q.sign_packing"] = (GGUF_TYPE_STRING, "msb_first")
    sorted_layers = sorted(lowbit_q_layer_indices)
    metadata["lowbit-q.layers"] = (GGUF_TYPE_ARRAY, (GGUF_TYPE_UINT32, sorted_layers))
    ...
    print(f"  Lowbit-Q layers: {sorted_layers}")


def main():
    parser = argparse.ArgumentParser(
        description="Convert standard GGUF to lowbit-Q GGUF format",
    )
    parser.add_argument("input", help="Input GGUF file path")
    parser.add_argument("output", help="Output lowbit-Q GGUF file path")
