...
struct DefaultInspectorView: View {
    let viewState: ThreeColumnViewState
    let actions: ThreeColumnActions
            ...
            content
                ...
                .toolbar {
                    if !viewState.toolbarsHidden {
                        ...
                        if !viewState.inspectorPresented {
                            ToolbarItem(placement: .topBarTrailing) {
                                Button {
                                    actions.toggleInspector()
                                } label: {
                                    Label(
                                        viewState.showInspectorLabel,
                                        systemImage: viewState.showInspectorIcon
                                    )
                                }
                                .accessibilityLabel(viewState.showInspectorAccessibilityLabel)
                            }
                        }
                        if let trailingToolbar {
                            ToolbarItemGroup(placement: .topBarTrailing) {
...
extension ThreePaneView where Sidebar == DefaultSidebarView, Detail == DefaultDetailView, Inspector == DefaultInspectorView {
    init(
        state: ThreeColumnState = ThreeColumnState(),
        icons: ThreeColumnButtonIcons = .init(),
        labels: ThreeColumnButtonLabels = .init()
...
extension ThreePaneView where Inspector == DefaultInspectorView {
    init<
