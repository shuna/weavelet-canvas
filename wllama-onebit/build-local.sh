#!/usr/bin/env bash
#
# build-local.sh — Build onebit-patched wllama WASM using local Emscripten.
#
# Prerequisites:
...
if [ ! -d "$FORK_DIR" ]; then
  ...
  echo "  bash wllama-onebit/setup.sh"
  exit 1
fi
...
echo "=== wllama-onebit local build ==="
...
ls -lh "$VENDOR_DIR/"
echo ""
echo "Onebit WASM ready at: $VENDOR_DIR/"
