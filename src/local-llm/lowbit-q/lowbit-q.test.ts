/**
 * End-to-end tests for the lowbit-Q quantization pipeline.
 *
 * Tests cover:
 * 1. FP16 ↔ FP32 conversion accuracy
 * 2. Q8_0 encode/dequantize round-trip
 * 3. GGUF parse → verify header, metadata, tensor info
 * 4. lowbit-Q decomposition correctness and quality (NMSE)
 * 5. lowbit-Q GGUF write → re-parse round-trip
 * 6. Full pipeline: synthetic Q8_0 GGUF → lowbit-Q GGUF → verify structure
 * 7. Full pipeline: synthetic F32 GGUF → lowbit-Q GGUF → verify structure
 */
...
import { GGMLType, GGUFValueType, LOWBIT_Q_VERSION_KEY, LOWBIT_Q_LAYERS_KEY, LOWBIT_Q_PACKING_KEY, LOWBIT_Q_SUFFIX_A, LOWBIT_Q_SUFFIX_B, LOWBIT_Q_SUFFIX_SIGN } from './types';
...
// ---------------------------------------------------------------------------
// 5. lowbit-Q decomposition
// ---------------------------------------------------------------------------

describe('lowbit-Q decomposition', () => {
...
// ---------------------------------------------------------------------------
// 6. Full pipeline: F32 GGUF → lowbit-Q GGUF → verify
// ---------------------------------------------------------------------------

describe('full pipeline (F32 source)', () => {
  it('converts F32 GGUF to lowbit-Q GGUF', () => {
    const { buffer } = buildToyModelGGUF({
    ...
    const result = convertToLowbitQ(buffer, { computeQuality: true });
  ...
  it('output contains lowbit-Q metadata', () => {
    const { buffer } = buildToyModelGGUF({
      weightType: GGMLType.F32,
      outFeatures: 8,
      inFeatures: 16,
    ...
    const result = convertToLowbitQ(buffer);
    const outHeader = parseGGUFHeader(result.data.buffer);

    // Check lowbit-Q metadata
    const version = outHeader.metadata.get(LOWBIT_Q_VERSION_KEY);
    expect(version).toBeDefined();
    expect(version!.value).toBe(1);

    const packing = outHeader.metadata.get(LOWBIT_Q_PACKING_KEY);
    expect(packing).toBeDefined();
    expect(packing!.value).toBe('msb_first');

    const layers = outHeader.metadata.get(LOWBIT_Q_LAYERS_KEY);
    expect(layers).toBeDefined();
    expect(Array.isArray(layers!.value)).toBe(true);
  });

  it('output contains lowbit-Q tensor triplets', () => {
    const { buffer } = buildToyModelGGUF({
      weightType: GGMLType.F32,
      outFeatures: 8,
      inFeatures: 16,
    ...
    // lowbit-Q triplet should be present
    expect(tensorNames).toContain('model.layers.0.self_attn.q_proj' + LOWBIT_Q_SUFFIX_A);
    expect(tensorNames).toContain('model.layers.0.self_attn.q_proj' + LOWBIT_Q_SUFFIX_B);
    expect(tensorNames).toContain('model.layers.0.self_attn.q_proj' + LOWBIT_Q_SUFFIX_SIGN);
  });

  it('lowbit-Q tensor dimensions are correct', () => {
    const outFeatures = 8;
    const inFeatures = 16;
    const { buffer } = buildToyModelGGUF({
      weightType: GGMLType.F32,
    ...
    const aT = outHeader.tensors.find(t => t.name.endsWith(LOWBIT_Q_SUFFIX_A));
    expect(aT).toBeDefined();
    expect(Number(aT!.dims[0])).toBe(outFeatures);
    expect(aT!.type).toBe(GGMLType.F16);

    const bT = outHeader.tensors.find(t => t.name.endsWith(LOWBIT_Q_SUFFIX_B));
    expect(bT).toBeDefined();
    expect(Number(bT!.dims[0])).toBe(inFeatures);
    expect(bT!.type).toBe(GGMLType.F16);

    const signT = outHeader.tensors.find(t => t.name.endsWith(LOWBIT_Q_SUFFIX_SIGN));
...
// ---------------------------------------------------------------------------
// 7. Full pipeline: Q8_0 GGUF → lowbit-Q GGUF → verify
// ---------------------------------------------------------------------------

describe('full pipeline (Q8_0 source)', () => {
  it('converts Q8_0 GGUF to lowbit-Q GGUF', () => {
    const { buffer } = buildToyModelGGUF({
    ...
    // Note: for tiny tensors, lowbit-Q GGUF may be larger due to header overhead.
...
describe('passthrough tensor type preservation', () => {
  ...
  it('preserves types with array metadata (mimicking real models)', () => {
    ...
    // Then call writeLowbitQGGUF DIRECTLY (not convertToLowbitQ) to test the
    ...
    // Build passthrough and lowbit-Q tensor sets manually
    ...
    for (const tensor of sourceHeader.tensors) {
      ...
      if (isWeightTensor(tensor.name)) {
        ...
        lowbitQTensors.set(tensor.name, {
    ...
    // lowbit-Q triplets
    const aT = outHeader.tensors.find(t => t.name.endsWith('.lowbit_q_a'));
    expect(aT).toBeDefined();
    expect(aT!.type).toBe(GGMLType.F16);

    const signT = outHeader.tensors.find(t => t.name.endsWith('.lowbit_q_sign'));
    expect(signT).toBeDefined();
...
describe('size reduction', () => {
  it('lowbit-Q GGUF is significantly smaller than F32 source', () => {
    const { buffer } = buildToyModelGGUF({
      weightType: GGMLType.F32,
      outFeatures: 32,
      inFeatures: 64,
  ...
  it('lowbit-Q GGUF is smaller than Q8_0 source', () => {
    ...
    // lowbit-Q: ~448 bytes for weight vs Q8_0: ~2176 bytes (64 blocks * 34)
    const ratio = result.convertedSize / result.originalSize;
...
describe('convertToLowbitQ with tensorFilter', () => {
  it('respects attention-only filter: only attn tensors converted', () => {
    ...
    expect(tensorNames).not.toContain('model.layers.0.self_attn.q_proj.weight');
    expect(tensorNames).toContain('model.layers.0.self_attn.q_proj.lowbit_q_a');
  });
  ...
  it('populates tensorRecords with correct fields', () => {
    ...
    expect(qProjRecord!.lowbitQSizeBytes).toBeGreaterThan(0);
