...
import type { LowbitQDecomposition } from './types';
...
/**
 * Decompose a weight matrix into lowbit-Q representation using SVID.
 *
 ...
 * @returns LowbitQDecomposition with a, b, and packed sign bits
 */
...
/**
 * Reconstruct a weight matrix from lowbit-Q decomposition.
 *
 * W_approx[i,j] = a[i] * sign[i,j] * b[j]
 * where sign[i,j] = +1 if bit is set, -1 otherwise.
 */
export function reconstruct(decomp: LowbitQDecomposition): Float32Array {
